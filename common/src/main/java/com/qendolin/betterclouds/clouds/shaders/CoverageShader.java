...
public class CoverageShader extends Shader {
    ...
    public static final String DEF_LONGVIEW = "_LONGVIEW_";
    ...
    public static CoverageShader create(ResourceManager manager, ShaderParameters shaderParameters) throws IOException {
        Map<String, String> defs = ImmutableMap.ofEntries(
                Map.entry(CoverageShader.DEF_SIZE_XZ_KEY, Float.toString(shaderParameters.configSizeXZ())),
                Map.entry(CoverageShader.DEF_SIZE_Y_KEY, Float.toString(shaderParameters.configSizeY())),
                Map.entry(CoverageShader.DEF_POSITIONAL_COLORING, shaderParameters.useStencilTextureFallback() ? "0" : "1"),
                Map.entry(CoverageShader.DEF_DISTANT_HORIZONS, shaderParameters.useDistantHorizonsCompat() ? "1" : "0"),
                Map.entry(CoverageShader.DEF_LONGVIEW, shaderParameters.useLongviewCompat() ? "1" : "0"),
                Map.entry(CoverageShader.DEF_WORLD_CURVATURE, Integer.toString(shaderParameters.worldCurvatureSize()))
        );
