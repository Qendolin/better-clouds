...
void main() {
    ...
    float scaleFalloff = mix(1.0, u_miscellaneous.x, pow(length(localWorldPosition.xz), 2.) / pow(u_bounding_box.z, 2.));
    ...
    float waveScale = texture(u_noise_texture, (localWorldPosition.xz + u_bounding_box.xy) / 4000.0 + vec2(u_time / 800f)).r;
    float smallWaves = texture(u_noise_texture, (localWorldPosition.zx + u_bounding_box.yx) / 1000.0 + vec2(u_time / 200f)).r * 1.8 - 0.9;
    waveScale = mix(mix(waveScale, 1.0, max(smallWaves, 0.0)), 0.0, max(-smallWaves, 0.0));
    float fDynScale = 1 - smoothstep(0., u_bounding_box.w / 4f, in_pos.y+0.5);
    ...
    pass_color.b = texture(u_noise_texture, localWorldPosition.xz / 1024.).g;
